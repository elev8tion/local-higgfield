...
from .mock_comfy_service import run_in_thread
...
def main() -> int:
    ...
    try:
        payload_dir = Path("/tmp/open-higgsfield-comfy-smoke")
        ...
        write_payload(
            video_payload,
            {"type": "video.generate", "prompt": "test comfy video", "params": {"model": "wan"}, "input_assets": []},
        )
        write_payload(
            lipsync_payload,
            {
                ...
                "prompt": "",
                "params": {"model": "latentsync"},
                "input_assets": [],
            },
        )

        env = dict(os.environ)
        env.update({"OPEN_HIGGSFIELD_COMFY_BASE_URL": base_url})

        video = run(
            [
                "python3",
                "-m",
                "backend.runners.comfy_video_runner",
                "--payload",
                str(video_payload),
                "--output",
                str(video_output),
        ...
        lipsync = run(
            [
                "python3",
                "-m",
                "backend.runners.comfy_lipsync_runner",
                "--payload",
                str(lipsync_payload),
                "--output",
                str(lipsync_output),
            ...
            json.dumps(
                {
                    ...
                    "video_stderr_tail": video.stderr[-400:],
                    "lipsync_stderr_tail": lipsync.stderr[-400:],
                },
                indent=2,
            )
        )
